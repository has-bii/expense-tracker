...
import { useDeleteExpenseMutation } from '../api/delete-expense'

const props = defineProps<{
  isOpen: Ref<boolean>
  close: () => void
  expense: Ref<Expense | null>
}>()

const mutation = useDeleteExpenseMutation()

const handleDelete = () => {
  if (!props.expense.value) return

  mutation.mutate(props.expense.value.id, {
    onSuccess: () => {
      props.close()
    },
  })
...
<template>
  <Dialog :open="isOpen.value" @update:open="close">
    <DialogContent>
      <DialogHeader>
        <DialogTitle>Delete expense: {{ expense.value?.description || 'this expense' }}</DialogTitle>
        <DialogDescription>
